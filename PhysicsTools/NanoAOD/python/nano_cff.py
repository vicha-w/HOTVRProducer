...
from PhysicsTools.NanoAOD.jets_hotvr_cff import *
...
#    hotvrjets=cms.InputTag("hotvrPFJets"),
)
...
simpleCleanerTable = cms.EDProducer("NanoAODSimpleCrossCleaner",
   ...
   tauName=cms.string("Tau"),photonName=cms.string("Photon")#, hotvrjetName=cms.string("HOTVRJet")
)
...
nanoSequenceCommon = cms.Sequence(
        nanoMetadata + hotvrjetSequence + jetSequence + muonSequence + tauSequence + boostedTauSequence + electronSequence + lowPtElectronSequence + photonSequence+vertexSequence+
        isoTrackSequence + jetLepSequence + # must be after all the leptons
        linkedObjects  + hotvrjetTables +
...
nanoSequenceFS = cms.Sequence(genParticleSequence + genVertexTables + particleLevelSequence + nanoSequenceCommon + jetMC + hotvrjetMC + muonMC + electronMC + lowPtElectronMC + photonMC + tauMC + boostedTauMC + metMC + ttbarCatMCProducers +  globalTablesMC + btagWeightTable + genWeightsTable + genVertexTable + genParticleTables + particleLevelTables + lheInfoTable  + ttbarCategoryTable )
